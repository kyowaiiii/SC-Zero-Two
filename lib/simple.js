            ...
            async value(m) {
                ...
                switch (m.messageStubType) {
                    ...
                    case WAMessageStubType.GROUP_PARTICIPANT_CHANGE_NUMBER:
                        // Log the stub for debugging if needed, but primarily extract mapping
                        console.log({
                        ...
                        try {
                            // Parse the JSON parameter which contains the mapping
                            // e.g. '{"id":"100798872690841@lid","phoneNumber":"6285174269046@s.whatsapp.net","admin":null}'
                            ...
                            if (param) {
                                ...
                                if (parsed.id && parsed.phoneNumber) {
                                    ...
                                    if (!conn.isLid) conn.isLid = {}
                                    // Store mapping: LID -> PhoneJid
                                    const lid = parsed.id
                                    const phone = parsed.phoneNumber
                                    conn.isLid[lid] = phone
                                    // Also store stripped version if needed
                                    // conn.isLid[lid.split('@')[0]] = phone
                                }
                            }
                        } catch (e) {
                            console.error('Failed to parse message stub param for LID mapping:', e)
                        }
                        break
                    default: {
                        console.log({
                        ...
                        if (message.message?.[mtype]?.contextInfo?.quotedMessage) {
                            ...
                            if ((remoteJid && remoteJid !== 'status@broadcast') && quoted) {
                                ...
                                const qM = {
                                    ...
                                    message: (() => {
                                        try {
                                            return JSON.parse(JSON.stringify(quoted))
                                        } catch (e) {
                                            return quoted
                                        }
                                    })(),
    ...
    return Object.defineProperties(proto.WebMessageInfo.prototype, {
        ...
        sender: {
            get() {
                ...
                // Collect potential user JIDs from available key fields
                const candidates = [
                    this.key?.participantAlt,
                    this.key?.participant,
                    this.key?.remoteJidAlt,
                    this.key?.remoteJid, // potentially chat/sender in DM
                    this.chat
                ].filter(Boolean).map(s => String(s).decodeJid())

                // 1. Prefer explicit Phone JID (@s.whatsapp.net)
                const phone = candidates.find(c => c.endsWith('@s.whatsapp.net'))
                if (phone) return this.conn.getJid(phone)

                // 2. Fallback to first available candidate (likely LID or Group)
                // Use getJid to attempt resolution if it's an LID
                return this.conn.getJid(candidates[0] || '')
            },
            enumerable: true
        },
        fromMe: {
